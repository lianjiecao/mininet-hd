...
class Partitioner:
    ...
    def loadtopo(self,topo, alg="kahip"):
        i=1
        self.pos={}
        self.switches={}
        self.hostToSw = {}
        ...
        for link in topo.links():
            ...
            if (topo.isSwitch(link[0]) and topo.isSwitch(link[1])):
                graphFile[self.switches[link[0]]].append(self.switches[link[1]])
            ...
            if topo.isSwitch(link[0]):
                ...
                graphFile[self.switches[link[0]]][0] = graphFile[self.switches[link[0]]][0] + int(topo.linkInfo(link[0],link[1])["bw"])
            if topo.isSwitch(link[1]):
                self.hostToSw[link[0]] = link[1]
                graphFile[self.switches[link[1]]][0] = graphFile[self.switches[link[1]]][0] + int(topo.linkInfo(link[0],link[1])["bw"])
            # else:
        ...
        #write header
        graphFile[0]=[len(self.switches),links,"11"]
        ret = ""
    ...
    def _write_to_file(self,pstr):
        with warnings.catch_warnings():
            ...
            filename=os.tempnam()
        self.logger.debug("KaHIP file: "+filename)
...
#                        self.partitions[switch_to_part[edge[0]]].addLink(edge[0],node,**self.topo.linkInfo(edge[0],node))
        ...
        print "KaHIP-addNodes:", time.time()-startT
        startT = time.time()
        ...
        print "KaHIP-addLinks:", time.time()-startT
